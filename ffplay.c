...
/*如果差值小于最小阈值，则不进行AV同步校正*/
#define AV_SYNC_THRESHOLD_MIN 0.04
/*如果差值超过最大阈值，则需要进行重新同步校正*/
...
typedef struct VideoState {
  ...
  int audio_stream;//audio_stream_index
  ...
  int audio_volume;
  int muted;
  struct AudioParams audio_src;//似乎和audio_tgt内容一样，是否后面会有差别？
#if CONFIG_AVFILTER
...
static int upload_texture(SDL_Texture **tex, AVFrame *frame,
  ...
  switch (sdl_pix_fmt) {
    case SDL_PIXELFORMAT_UNKNOWN:
      ...
      *img_convert_ctx =
              sws_getCachedContext(*img_convert_ctx, frame->width, frame->height,
...
/*
  获取的是pts+系统时增量：c->pts + (curTime-setTime)
*/
...
static double compute_target_delay(double delay, VideoState *is) {
  ...
  if (get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER) {
    ...
    //（视频设置时的pts + 视频开始渲染到现在的所经过的时间） - （音频的pts + 音频渲染时间到现在所进过的时间）;
    diff = get_clock(&is->vidclk) - get_master_clock(is);

    sync_threshold = FFMAX(AV_SYNC_THRESHOLD_MIN, FFMIN(AV_SYNC_THRESHOLD_MAX, delay));

    //两个时钟的差异，帧延迟时间在最大可接受范围内3600s，则可以对视频进行动态同步处理
    if (!isnan(diff) && fabs(diff) < is->max_frame_duration) {
      //sync_threshold[0.04,0.1]:阈值在[40ms,100ms]
      if (diff <= -sync_threshold) {
        //视频慢于音频，需要加快播放，该帧的延迟需要减少
        delay = FFMAX(0, delay + diff);
      } else if (diff >= sync_threshold && delay > AV_SYNC_FRAMEDUP_THRESHOLD) {
        //视频播放太快，而且当前帧目标持续时间可以很长，则直接进行延迟等待音频
        delay = delay + diff;
...
static void video_refresh(void *opaque, double *remaining_time) {
  ...
  retry:
    if (frame_queue_nb_remaining(&is->pictq) == 0) {
      ...
      //获取上一帧frame（其实就是当前正在渲染的frame）
      lastvp = frame_queue_peek_last(&is->pictq);
      //获取下一帧frame
      vp = frame_queue_peek(&is->pictq);

      /*
      如果下一帧的serial和视频队列里的serial不同,则直接将当前画面正在渲染的frame移除队列
      出现这种情况是，当前发生了seek操作（可以看packet_queue_flush里的函数实现），需要将画面快速切换到seek后的frame上
      */
      ...
      //计算上一帧视频理论上应该显示多久，其实就是两个frame的pts差值
      last_duration = vp_duration(is, lastvp, vp);

      //通过和基准时钟对比，获得真正需要延迟的时间
      delay = compute_target_delay(last_duration, is);

      //获取当前时间
      time = av_gettime_relative() / 1000000.0;

      //表示当前还没到渲染结束的时间点，需要继续显示当前帧
      if (time < is->frame_timer + delay) {
        //is->frame_timer + delay - time : 当前帧渲染剩余时间
        //remaining_time 每次调用video_refresh函数时都会重置成0.01然后和渲染剩余时间计算出一个最小值
        *remaining_time = FFMIN(is->frame_timer + delay - time, *remaining_time);
        //从这里到直接到display后，由于is->force_refresh参数是0，其实相当什么也没做，而是sleep（remaining_time）后再次循环调用video_refresh
        goto display;
      }
      //更新帧播放时间轴，上一帧已经播放结束
      is->frame_timer += delay;
      //代码运行到这里：time - is->frame_timer > 0
      //视频播放时间轴落后当前时间轴超过100ms
      //如果不进行重新校对，则会出现丢帧现象
      ...
      //如果frameQueue里只有一帧画面待render，则不需要丢帧
      if (frame_queue_nb_remaining(&is->pictq) > 1) {
        Frame *nextvp = frame_queue_peek_next(&is->pictq);
        duration = vp_duration(is, vp, nextvp);
        //当前时间超过了下一帧的渲染结束时间点，该帧画面跳过，不需要渲染，直接选取下一帧画面
  ...
  if (autorotate) {
    ...
    if (!displaymatrix)
      displaymatrix = (int32_t *) av_stream_get_side_data(
              is->video_st, AV_PKT_DATA_DISPLAYMATRIX, NULL);
...
static int audio_decode_frame(VideoState *is) {
...
#endif
    //从队列中获取一帧可以渲染的数据
    if (!(audioFrame = frame_queue_peek_readable(&is->sampq))) {
      return -1;
    }
    //移除当前正在渲染的frame
    frame_queue_next(&is->sampq);
  } while (audioFrame->serial != is->audioq.serial);

  //计算这个audioFrame里的sample占用多少字节
  data_size = av_samples_get_buffer_size(NULL, audioFrame->frame->ch_layout.nb_channels,
                                         audioFrame->frame->nb_samples, audioFrame->frame->format, 1);
  //每个channel的采样点数
  //这个值，如果是音频作为主时钟，则直接的等于audioFrame->frame->nb_samples
  //这个函数不建议分析，太复杂，而已没什么必要，可以认为wanted_nb_samples = frame->nb_samples
  wanted_nb_samples = synchronize_audio(is, audioFrame->frame->nb_samples);

  //判断是否需要新创建重采样swr_ctx
  if (audioFrame->frame->format != is->audio_src.fmt || av_channel_layout_compare(&audioFrame->frame->ch_layout, &is->audio_src.ch_layout) ||
      audioFrame->frame->sample_rate != is->audio_src.freq || (wanted_nb_samples != audioFrame->frame->nb_samples && !is->swr_ctx)) {

    swr_free(&is->swr_ctx);
    swr_alloc_set_opts2(&is->swr_ctx, &is->audio_tgt.ch_layout,
                        is->audio_tgt.fmt, is->audio_tgt.freq,
                        &audioFrame->frame->ch_layout, audioFrame->frame->format,
    ...
    if (!is->swr_ctx || swr_init(is->swr_ctx) < 0) {
      av_log(NULL, AV_LOG_ERROR,
             ...
             audioFrame->frame->sample_rate, av_get_sample_fmt_name(audioFrame->frame->format),
             audioFrame->frame->ch_layout.nb_channels, is->audio_tgt.freq,
    ...
    //audio_src和frame的参数保持一致，当参数不一致的时候，则会重新创建swr_ctx，否则只会创建一次
    if (av_channel_layout_copy(&is->audio_src.ch_layout, &audioFrame->frame->ch_layout) < 0) {
      return -1;
    }
    is->audio_src.freq = audioFrame->frame->sample_rate;
    is->audio_src.fmt = audioFrame->frame->format;
  }
  ...
  if (is->swr_ctx) {
    ...
    //重采样后的采样点个数 = 声卡目标采样率/音频源采样率 * 原始采样点个数
    //这个值只是理论上重采样后采样点的个数，实际重采样后的sample点数，以具体重采样输出为准，可能会和理论计算的值有出入
    //这个会值用于计算存放重采样后的数据缓存大小，因此最好给的比理论计算要大一些，否则可能会丢失采样点，因此这里+256
    int out_count = (int64_t) wanted_nb_samples * is->audio_tgt.freq / audioFrame->frame->sample_rate + 256;
    ...
    //这里只有是音频同步到别的时钟时才会出现差异
    if (wanted_nb_samples != audioFrame->frame->nb_samples) {
      //如果原来一帧数据点是1024，wanted_nb_samples是1000，重采样后数据点和采样率都会发生变化
      //结果可能导致每个sample的pts对不上，因此需要对每个sample的pts做补偿
      if (swr_set_compensation(is->swr_ctx,
                               (wanted_nb_samples - audioFrame->frame->nb_samples) *
                                       is->audio_tgt.freq / audioFrame->frame->sample_rate,
                               wanted_nb_samples * is->audio_tgt.freq /
                                       audioFrame->frame->sample_rate) < 0) {
    ...
    if (!is->audio_buf1)
      return AVERROR(ENOMEM);
    len2 = swr_convert(is->swr_ctx, out, out_count, in, audioFrame->frame->nb_samples);
    if (len2 < 0) {
      av_log(NULL, AV_LOG_ERROR, "swr_convert() failed\n");
      return -1;
    }

    //实际输出等于out_count的时候：代表两种情况：
    //1.重采样后的采样点正好等于out_count.
    //2.可能比out_count大,但是由于buffer的限制，导致最终输出和out_count一样
    if (len2 == out_count) {
    ...
    is->audio_buf = audioFrame->frame->data[0];
  ...
  // printf("Sivin:af->pts = %lf \n", af->pts);
  if (!isnan(audioFrame->pts)) {
    //表示该audiofrme播放完后的pts
    is->audio_clock = audioFrame->pts + (double) audioFrame->frame->nb_samples / audioFrame->frame->sample_rate;
  } else {
    is->audio_clock = NAN;
  }
  is->audio_clock_serial = audioFrame->serial;
#ifdef DEBUG
  {
...
//stream：指向audioDataBuffer的指针，len就是之前计算的audio_hw_buf_size=8192 = 2048 * 4（一次需要2048个sample，每个sample4个字节）
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len) {
  ...
  while (len > 0) {
    //audio_buf里的数据取完了，需要加载新的audio数据
    if (is->audio_buf_index >= is->audio_buf_size) {
      //获取一帧解码后的音频数据：更新is->audio_buf
      //返回audio_buf的大小
      audio_size = audio_decode_frame(is);

      //没有获取到可用于渲染的audioFrame
      if (audio_size < 0) {
    ...
    if (!is->muted && is->audio_buf && is->audio_volume == SDL_MIX_MAXVOLUME) {
      ...
      if (!is->muted && is->audio_buf) {
        //这个函数的原意：src的数据调整音量后加到dest数据上，同时对dst上的数据做了调整
        //这里当stream为空时，stream = audio_buf调整音量后的值
  ...
  if (!isnan(is->audio_clock)) {
    /*声卡有两个缓冲区，当一个缓冲区消耗结束时，会回调给前端填充数据，同时切换另一个缓冲区消费数据.
      解码是按着一个完整frame为单位进行的，但是声卡消费数据却不一定能每次都消费一个完整frame数据。
      那么就存在如下情况，声卡正在播放的缓冲区数据时上一个frame里的，当前正在填充的缓冲区数据是当前frame的
      而当前frame的数据没有被全部填充，还有剩余就是：audio_write_buf_size。
      audio_clock：表示当前frame播放完毕时的pts，而我们需要计算的是当前正在播放的采样点的pts，因此需要向前回推
      render_duration= (2 * audio_hw_buf_size + audio_write_buf_size) / is->audio_tgt.bytes_per_sec：表示硬件缓冲区里的数据加上还剩余还未填充的数据能够播放多久
      当前正在渲染的pts = audio_clock - render_duration
      audio_callback_time:本质上是计算当前正在播放采样点的系统时间，但是由于读取数据可能存在耗时，而在这段耗时期间，声卡并不受到cpu耗时的影响，此时已经在消费数据了。
      因此如果在调用set_clock_at时获取系统时间戳，实际上并不是刚开始播放时的sample对应的系统时间戳，所以这里为了减少误差，在sdl_audio_callback回调时提前获取了时间戳
    */
...
static int audio_open(void *opaque, AVChannelLayout *wanted_channel_layout,
  ...
  wanted_spec.silence = 0;//音频是时域信号，有震动才能有声音，就是有值的变化才有声音，设置相同的值后，就是静默音，一般是每个字节都设置为0或者1，为静默音

  //av_log2:计算一个整数的二进制最高有效位是多少，从0开始计算，例如（7=0b111）最高有效位是2，（8=0b1000）最高有效位是3
  //wanted_spec.freq / SDL_AUDIO_MAX_CALLBACKS_PER_SEC： 48000/30 = 1600，表示每次回调送入多少个采样点，这个值无法整除时向0取整
  //2 << av_log2(wanted_spec.freq / SDL_AUDIO_MAX_CALLBACKS_PER_SEC)//计算结果按2的整数倍对齐（SDL要求采样点必须是2的整数倍）
  //由于内部整除结果存在向0取整问题，导致计算结果偏小，所以对齐时是向高对齐：[4，8）-> 8，[8,16) -> 16
  //所以这里的1600在[1024,2048）内，因此计算结果是2048
  wanted_spec.samples = FFMAX(SDL_AUDIO_MIN_BUFFER_SIZE, 2 << av_log2(wanted_spec.freq / SDL_AUDIO_MAX_CALLBACKS_PER_SEC));

  wanted_spec.callback = sdl_audio_callback;
  wanted_spec.userdata = opaque;

  //返回0失败，spec会返回设备支持的spec
  while (!(audio_dev = SDL_OpenAudioDevice(NULL, 0, &wanted_spec, &spec,
    ...
    //next_nb_channels数组的index映射成一个新的channels，尝试所有还无法打开audio设备，则返回失败
  ...
  return spec.size;//8192 => 每次samples * frame_size = 2048 *4 = 8192;
}
...
static int stream_component_open(VideoState *is, int stream_index) {
  ...
  switch (avctx->codec_type) {
...
#if CONFIG_AVFILTER
    {
      ...
      //由于我们无法精确的对音频fifo，所以我们需要校正，这个值在不使用音频作为同步基准时使用
      //含义：音频硬件一次缓冲可以播放多少秒
...
static int read_thread(void *arg) {
  ...
  for (i = 0; i < ic->nb_streams; i++) {
    ...
    // 有的视频存在多个声音流，例如国语和粤语，在播放时可以指定播放哪种声音轨道，这里就是选择要播放的index，存放到st_index中
  ...
  for (;;) {
    ...
    switch (event.type) {
      case SDL_KEYDOWN:
        ...
        // If we don't yet have a window, skip all key events, because read_thread
